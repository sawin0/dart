// Given a string containing digits from 2-9 inclusive, return all possible letter combinations that the number could represent. Return the answer in any order.

// A mapping of digits to letters (just like on the telephone buttons) is given below. Note that 1 does not map to any letters.

// Example 1:

// Input: digits = "23"
// Output: ["ad","ae","af","bd","be","bf","cd","ce","cf"]

// Example 2:

// Input: digits = ""
// Output: []

// Example 3:

// Input: digits = "2"
// Output: ["a","b","c"]

// source: https://leetcode.com/problems/letter-combinations-of-a-phone-number

void main() {
  Solution solution = Solution();
  print(solution.letterCombinations(
      "23")); // ["ad","ae","af","bd","be","bf","cd","ce","cf"]
  print(solution.letterCombinations("")); // []
  print(solution.letterCombinations("2")); // ["a","b","c"]
}

class Solution {
  List<String> letterCombinations(String digits) {
    // Map of digits to their corresponding letters
    Map<String, List<String>> data = {
      '2': ['a', 'b', 'c'],
      '3': ['d', 'e', 'f'],
      '4': ['g', 'h', 'i'],
      '5': ['j', 'k', 'l'],
      '6': ['m', 'n', 'o'],
      '7': ['p', 'q', 'r', 's'],
      '8': ['t', 'u', 'v'],
      '9': ['w', 'x', 'y', 'z'],
    };

    // Edge case: if digits is empty, return an empty list
    if (digits.isEmpty) return [];

    // Initialize result with an empty string to start combinations
    List<String> result = [''];

    // Iterate through each digit and build combinations iteratively
    for (String digit in digits.split('')) {
      List<String>? letters = data[digit]; // Get letters for the digit
      if (letters == null) continue; // Skip invalid digits

      // Build new combinations by appending each letter to existing results
      List<String> newResult = [];
      for (String combination in result) {
        for (String letter in letters) {
          newResult.add(combination + letter);
        }
      }
      result = newResult; // Update result with new combinations
    }

    return result;
  }
}
